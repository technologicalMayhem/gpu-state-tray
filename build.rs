...
fn generate_icon_consts() {
    ...
    let image_data = vec![
        const_declaration!(IMAGE_OFF = process_image("off.png")),
        const_declaration!(IMAGE_ACTIVE = process_image("active.png")),
    ]
    .join("\n");
...
fn process_image(filename: &str) -> Vec<u8> {
    ...
    let dyn_image = image::open(&path).expect(&format!("Could not find '{path:?}'."));
    let image = dyn_image
        .as_rgba8()
